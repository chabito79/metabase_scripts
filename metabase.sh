...
METABASE_INSTALL=/opt/metabase/current

# where is the Metabase jar located?
METABASE=$METABASE_INSTALL/lib/metabase.jar
...
start() {
  # Setup iptables prerouting rule
  ...
  local CMD="nohup java $MB_JAVA_ARGS -jar \"$METABASE\" &> \"$LOGFILE\" & echo \$!"
